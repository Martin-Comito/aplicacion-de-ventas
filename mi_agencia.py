...
# CONEXIÓN GOOGLE GEMINI (IA)
# Intenta conectar automáticamente desde los secretos
api_key = None
try:
...
if st.session_state.usuario is None:
    ...
    if user_from_cookie: st.session_state.usuario = user_from_cookie
...
with st.sidebar:
    ...
    st.write("---")
    # VISUALIZADOR DE ESTADO IA
    if api_key:
        st.success("🤖 IA Conectada")
    else:
        st.warning("⚠️ IA Desconectada")
        st.caption("Agrega [google] api_key en Secrets o ingresala manual:")
        api_key_manual = st.text_input("Key Manual", type="password")
        if api_key_manual:
            ...
            if st.form_submit_button("Guardar Cliente"):
                if nombre and empresa:
                    try:
                        ...
                        st.success("Cliente guardado."); time_module.sleep(1); st.rerun()
    ...
    if res.data:
        for c in res.data:
            with st.container(border=True):
                ...
                if ROL == 'DIRECTOR' and 'agencia_usuarios' in c: titulo += f" | 👤 *{c['agencia_usuarios']['nombre_completo']}*"
        ...
        if mapa_clientes:
            ...
            if st.form_submit_button("Agendar"):
                ...
                supabase.table("agencia_citas").insert({
                    "usuario_id": ID_USER, "cliente_id": mapa_clientes[cli_sel], "fecha_hora": dt_str, "motivo": motivo
                }).execute()
...
# VISTA 3: IA GENERADOR
    ...
    if mapa_cli:
        ...
        if st.button("🚀 GENERAR PROPUESTA", type="primary"):
            ...
            else: st.warning("Falta API Key o Problema. Revisa que la IA esté conectada en la barra lateral.")
            ...
            with st.container(border=True):
                ...
                if st.button("💾 Guardar Proyecto"):
                    supabase.table("agencia_proyectos").insert({
                        ...
                        "solucion_ia": st.session_state.ia_result, "fecha_limite_entrega": str(fecha_entrega)
                    }).execute()
                    st.success("Proyecto Guardado."); del st.session_state.ia_result
    else: st.warning("Carga clientes primero.")

# VISTA 4: ESTADO DE PROYECTOS
    ...
    if proys.data:
        for p in proys.data:
            ...
            if ROL == 'DIRECTOR' and 'agencia_usuarios' in p: titulo += f" (Vendedor: {p['agencia_usuarios']['nombre_completo']})"
            ...
            with st.expander(titulo):
                ...
                if not st.session_state[key_edit]:
                    ...
                    if col_btn.button("✏️ Editar", key=f"btn_edit_{p['id']}"):
                        st.session_state[key_edit] = True; st.rerun()
                    st.write(f"**Problema:** {p['problema_cliente']}")
                    st.markdown("**Solución:**"); st.markdown(p['solucion_ia']); st.divider()
                    ...
                    if nuevo_est != p['estado_proyecto']:
                        ...
                        st.toast("Actualizado"); time_module.sleep(0.5); st.rerun()
                else:
                    ...
                    if c_save.button("💾 Guardar", key=f"save_{p['id']}", type="primary"):
                        supabase.table("agencia_proyectos").update({"problema_cliente": new_prob, "solucion_ia": new_sol}).eq("id", p['id']).execute()
                        st.session_state[key_edit] = False; st.success("Actualizado"); st.rerun()
                    if c_cancel.button("❌ Cancelar", key=f"cancel_{p['id']}"):
                        st.session_state[key_edit] = False; st.rerun()
